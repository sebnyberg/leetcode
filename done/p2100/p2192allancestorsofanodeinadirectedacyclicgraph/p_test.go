...
func getAncestors(n int, edges [][]int) [][]int {
	...
	for len(curr) > 0 {
		...
		for _, idx := range curr {
			// Remove from nearby edges, increment number of ancestors
			for _, adj := range adj[idx] {
				indeg[adj]--
				for a := range ancestors[idx] {
					ancestors[adj][a] = struct{}{}
				}
				ancestors[adj][idx] = struct{}{}
				if indeg[adj] == 0 {
	...
	// Sort and de-duplicate ancestors
	ancestorList := make([][]int, n)
	for i, m := range ancestors {
		for k := range m {
			ancestorList[i] = append(ancestorList[i], k)
		}
		sort.Ints(ancestorList[i])
	}
	return ancestorList
}
