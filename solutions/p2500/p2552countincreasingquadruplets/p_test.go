package p2552countincreasingquadruplets
...
func countQuadruplets(nums []int) int64 {
	// "When you got a segtree, everything looks like a nail."

	n := len(nums)

	add := func(a, b int) int { return a + b }

	left, _ := NewSegtree(0, add)
	left.Init(make([]int, n+1))

	right, _ := NewSegtree(0, add)
	right.Init(make([]int, n+1))

	var res int
	for j := 1; j < n-2; j++ {
		left.Update(nums[j-1], 1)
		if j > 1 {
			right.Reset(-1)
		}
		for k := n - 2; k > j; k-- {
			right.Update(nums[k+1], 1)
			if nums[k] < nums[j] {
				l := left.QueryRange(0, nums[k]-1)
				r := right.QueryRange(nums[j]+1, n)
				res += l * r
			}
		}
	}
