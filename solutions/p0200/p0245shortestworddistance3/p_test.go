package p0245shortestworddistance
...
func Test_shortestWordDistance(t *testing.T) {
	for _, tc := range []struct {
		...
		{[]string{"practice", "makes", "perfect", "coding", "makes"}, "makes", "makes", 3},
	} {
		t.Run(fmt.Sprintf("%+v", tc.wordsDict), func(t *testing.T) {
			require.Equal(t, tc.want, shortestWordDistance(tc.wordsDict, tc.word1, tc.word2))
		})
	}
}
